...
- 复制内置预设或从空白创建，编辑阶段顺序、条件、产物、资源与提交设置；支持 JSON 导入导出。
- 自定义流程的人工确认、验证和审核绑定当前阶段，已有任务保持原快照与版本。
- 配置保存保留 verify_command 等项目字段，使用摘要与文件版本检查并发修改；页面内对话框保护未保存内容。
- 提交钩子使用任务快照解析自定义消息格式，有快照时不依赖当前项目配置；更新插件后需重新 install_hook。
- 同步 README、HTML 手册、内置技能和配置示例；增加工具、客户端编解码、保存与钩子的行为回归。

验证明细见[自定义流程测试记录](testing/custom-workflows/测试执行记录.md)。
