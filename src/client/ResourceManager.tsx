...
const sourceText = (source: string) => source === 'bundled' ? '内置' : source === 'codex-project' ? '项目 · Codex' : source.startsWith('user') ? '个人' : '项目'
...
export function ResourceManager({ workspace, remote, kind, onConfigureSkill }: { workspace: string; remote: TaskEngineRemote; kind: 'skill' | 'rule'; onConfigureSkill?: (ref: ResourceRef) => void }) {
  ...
  const readonly = editing !== null && editing !== 'new' && (levelOf(editing) === 'bundled' || editing.source === 'codex-project')
  ...
  const edit = async (row: Row | 'new', view = false) => {
    ...
    try {
      const r = kind === 'skill' ? await remote.readSkill({ name: row.name, level: row.source === 'codex-project' ? 'codex-project' : lvl, path: workspace }) : await remote.readRule({ name: row.name, level: lvl, path: workspace })
      if (!r.ok) throw new Error(fail(r))
  ...
  return h('section', { className: 'te-resource' },
    ...
    loading ? h('div', { className: 'te-empty', role: 'status' }, '正在加载…') : visible.length === 0 ? h('div', { className: 'te-empty' }, h('h3', null, query || filter !== 'all' ? '没有匹配的结果' : `还没有${noun}`), h('p', null, '调整筛选，或使用右上角按钮添加。')) : h('div', { className: 'te-resource-grid' }, ...visible.map(row => h('article', { className: 'te-resource-card', key: row.source + '/' + row.name },
      ...
      h('div', { className: 'te-actions' }, kind === 'skill' && onConfigureSkill ? h(Button, { variant: 'primary', size: 'sm', onClick: () => onConfigureSkill({ source: row.source === 'codex-project' ? 'codex-project' : levelOf(row), name: row.name }) }, '配置规则') : null, h(Button, { variant: 'outline', size: 'sm', onClick: () => { void edit(row, true) } }, '查看'), levelOf(row) === 'bundled' ? h(Button, { variant: 'ghost', size: 'sm', disabled: busy, onClick: () => { void copyTemplate(row) } }, '以此为模板新建') : row.source !== 'codex-project' ? h(Button, { variant: 'ghost', size: 'sm', onClick: () => { void edit(row) } }, '编辑') : null, levelOf(row) !== 'bundled' && row.source !== 'codex-project' ? h('button', { className: 'te-delete', onClick: () => { setDeleting(row); setError('') } }, '删除') : null)))),
    ...
    editing ? h(ResourceModal, { title: editing === 'new' ? `新建${noun}` : `${viewing ? '查看' : '编辑'}${noun} · ${name}`, onClose: close, footer: h('div', { className: 'te-actions' }, h(Button, { variant: 'ghost', disabled: busy, onClick: close }, '关闭'), editing !== 'new' && levelOf(editing) === 'bundled' ? h(Button, { variant: 'outline', disabled: busy, onClick: () => { void copyTemplate(editing as Row) } }, '以此为模板新建') : null, !readonly ? h(Button, { variant: 'outline', disabled: busy, onClick: () => setViewing(!viewing) }, viewing ? '编辑内容' : '预览') : null, !readonly ? h(Button, { variant: 'primary', disabled: busy || !name.trim() || !body.trim() || (kind === 'skill' && !description.trim()), onClick: () => { void save() } }, busy ? '处理中…' : '保存') : null) },
      !readonly ? targets() : h('p', { className: 'te-badge' }, (editing as Row).source === 'codex-project' ? 'Codex 项目技能 · 正文请在原目录编辑' : '内置样本 · 只读'), templateSource ? h('p', null, `以「${templateSource.name}」为模板复制正文。新资源的完成凭证、规则和阶段绑定需单独配置，保存本身不会启用它。`) : null, status(),
