...
| **流程配置** | 选择预设或自定义阶段、条件、产物与提交策略，支持导入导出。 |
...
预设不够用？点击 **复制当前预设并自定义**，按你的习惯增删和排序阶段，配置确认、验证、审核与产物要求，自由挂载技能和规则。支持从空白创建及 JSON 导入导出，详见[自定义流程指南](docs/configuration.md)。
...
| 版本变化与验证记录 | [更新日志](docs/CHANGELOG.md) · [自定义流程测试报告](docs/testing/custom-workflows/测试报告.md) |
